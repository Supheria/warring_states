...
partial class ArchiveSelector
{
    ...
    private void RollReDraw()
    {
        ...
        g.FillRectangle(brush, new(RollItemsRect.Left, RollRect.Bottom - RollPadding, RollItemsRect.Width, RollPadding));
        brush.Color = BackColor;
        g.FillRectangle(brush, new(RollItemsRect.Left, Top, RollItemsRect.Width, Padding.Height));
        g.FillRectangle(brush, new(RollItemsRect.Left, RollRect.Bottom + RollPadding, RollItemsRect.Width, Padding.Height));
    ...
    private void RollReSize()
    {
        ...
        RollItemToShowCount = height / RollItemHeight;
        if (itemCount < RollItemToShowCount)
        {
            RollItemToShowCount = itemCount;
            height = RollItemToShowCount * RollItemHeight;
        }
        else
        {
            if (height % RollItemHeight is not 0)
                RollItemToShowCount++;
